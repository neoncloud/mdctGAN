...
class MDCT(torch.nn.Module):
    ...
    def mdct(
        ...
        **kwargs
        ...
        def cmdct(x, odd=True):
            ...
            if odd:
                outlen = N
                pre_twiddle = torch.exp(-1j * np.pi *
                                        torch.arange(N * 2) / (N * 2)).to(self.device)
                offset = 0.5
            else:
                outlen = N + 1
                pre_twiddle = 1.0
    ...
    def forward(self, x):
        x = self.mdct(x=x, window_function=self.window_function,
                      step_length=self.step_length, n_fft=self.n_fft, center=self.center, padding=0)
        return x
...
class IMDCT(torch.nn.Module):
    ...
    def imdct(
        ...
        **kwargs
        ...
        def icmdct(X, odd=True):
            ...
            pre_twiddle = (torch.exp(1j * np.pi * n0 *
                           torch.arange(N * 2) / N)).to(self.device)
    ...
    def forward(self, X):
        X = self.imdct(X=X, window_function=self.window_function, step_length=self.step_length,
                       n_fft=self.n_fft, padding=0, out_length=self.out_length)
        return X
...
class MDCT4(torch.nn.Module):
    ...
    def __init__(self, n_fft=2048, hop_length=None, win_length=None, window=None, center=True, pad_mode='constant', device='cuda') -> None:
        ...
        assert self.win_length <= self.n_fft, 'Window lenth %d should be no more than fft length %d' % (
            self.win_length, self.n_fft)
        assert self.hop_length <= self.win_length, 'You hopped more than one frame'

        self.exp1 = torch.exp(-1j*torch.pi/self.n_fft*torch.arange(start=0,
                              end=self.n_fft, step=1, dtype=torch.float64, device=self.device))
        self.exp2 = torch.exp(-1j*(torch.pi/(2*self.n_fft)+torch.pi/4)*torch.arange(
            start=1, end=self.n_fft, step=2, dtype=torch.float64, device=self.device))
    ...
    def forward(self, signal, return_frames: bool = False):
        ...
        signal = signal.unfold(
            dimension=-1, size=self.win_length, step=self.hop_length)
        ...
        if self.n_fft > self.win_length:
            signal = pad(signal, (0, self.n_fft-self.win_length),
                         mode='constant')
...
class IMDCT4(torch.nn.Module):
    def __init__(self, n_fft=2048, hop_length=None, win_length=None, window=None, center=True, pad_mode='constant', out_length=None, device='cuda') -> None:
        ...
        self.exp1 = torch.exp(-1j*(torch.pi/(2*self.n_fft)+torch.pi/4)*torch.arange(
            start=1, end=self.n_fft, step=2, dtype=torch.float64, device=self.device))
        self.exp2 = torch.exp(-1j*torch.pi/(2*self.n_fft)*torch.arange(
            start=0, end=2*self.n_fft, step=2, dtype=torch.float64, device=self.device))

    def forward(self, signal, return_frames: bool = False):
        assert signal.dim() == 3, 'Only tensors shaped in BHW are supported, got tensor of shape %s' % (
            str(signal.size()))
        assert signal.size(
        )[-1] == self.n_fft//2, 'The last dim of input tensor should match the n_fft. Expected %d ,got %d' % (self.n_fft, signal.size()[-1])
        ...
        signal = 4/self.n_fft*fold(signal.transpose_(-1, -2), kernel_size=(
            1, self.win_length), stride=(1, self.hop_length), output_size=(1, out_len))

        if self.center:
            # extract the middle part
            signal = signal[..., self.win_length//2:-self.win_length//2]
        signal = signal if self.out_length is None else signal[...,
                                                               :self.out_length]
        return signal, frames
...
class FastMDCT4(torch.nn.Module):
    '''
    Fast implementation of MDCT. Port to PyTorch by Chenhao Shuai
...
class FastIMDCT4(torch.nn.Module):
    def __init__(self, n_fft: Optional[int] = 2048, hop_length: Optional[int] = None, win_length: Optional[int] = None, window: Union[torch.Tensor, np.ndarray, list, Callable, None] = None, center: bool = True, pad_mode: str = 'constant', out_length: Optional[int] = None, device: str = 'cuda') -> None:
        ...
        self.window = (4.0*self.make_sign()*self.window /
                       self.n_fft).to(torch.float32).contiguous()
